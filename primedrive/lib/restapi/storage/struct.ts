import { AuthorizationType, AwsIntegration, IAuthorizer, LambdaIntegration, RestApi } from "aws-cdk-lib/aws-apigateway";
...
export class StorageResource extends Construct {
    constructor(
        ...
        objectRes.addMethod("GET",
            new AwsIntegration({
                service: "s3",
                integrationHttpMethod: "GET",
                path: bucketName + `/users/{sub}/{key}?attributes`,
                options: {
                    credentialsRole,
                    requestParameters: {
        ...
        objectRes.addMethod("DELETE", new AwsIntegration({
            service: "s3",
        ...
        presignedRes.addMethod("GET", new LambdaIntegration(presigner,{
            requestParameters:{
                "integration.request.querystring.sub": "context.authorizer.claims.sub",
