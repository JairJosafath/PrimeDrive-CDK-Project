...
import { aws_iam, aws_lambda, aws_s3, aws_s3_notifications, CfnOutput as output, RemovalPolicy } from "aws-cdk-lib";
...
export class PrimeDriveStorage extends Construct {
    ...
    constructor(scope: Construct, id: string, stateMachine: StateMachine) {
        ...
        const s3TriggerFunction = new aws_lambda.Function(this, "LambdaFunction", {
            ...
            code: aws_lambda.Code.fromAsset(path.join(__dirname, '../functions/s3Trigger')),
            environment: {
        ...
        this.bucket.addEventNotification(aws_s3.EventType.OBJECT_CREATED,
            new aws_s3_notifications.LambdaDestination( s3TriggerFunction ),
            { prefix: "users/" });

            this.presignedUrlFunction = new aws_lambda.Function(this, "PresignedUrlFunction", {
                runtime: aws_lambda.Runtime.NODEJS_20_X,
                handler: "index.handler",
                code: aws_lambda.Code.fromAsset(path.join(__dirname, '../functions/preSignedUrlGenerator')),
                environment:{
                ...
                role: new aws_iam.Role(this, "PresignedUrlFunctionRole", {
                    ...
                    managedPolicies: [
                        ...
                        aws_iam.ManagedPolicy.fromAwsManagedPolicyName('AmazonCognitoPowerUser')
                    ],
                })
            })
